...
const App = ({ role = 'teacher' }) => {
  const { t, i18n } = useTranslation(['common', 'neural']);
...
const saveStudentName = () => {
  ...
  let demoIcon = null;
  let demoLabel = t('neural.notSelected', { defaultValue: 'Μη επιλεγμένο' });
  ...
  if (activeActivity === '4' && effectiveLinearDemoIndex !== undefined && DATASETS[safeDisplayDataset]?.linear_demos) {
    ...
    if (selectedDemo) {
      ...
      demoLabel = selectedDemo.example || t('neural.notSelected', { defaultValue: 'Μη επιλεγμένο' });
    }
  }
  ...
  useEffect(() => {
    if (isStudent) {
      setLessonActivityTitle(getNeuralActivityTitle(lessonActivity));
    }
  }, [isStudent, lessonActivity, i18n.language]);
  ...
  const separableLabel = effectiveSeparable === null
    ? t('neural.separatorOff', { defaultValue: 'Διαχωρισμός: -' })
    : effectiveSeparable
      ? t('neural.separatorYes', { defaultValue: 'Διαχωρισμός: ✅' })
      : t('neural.separatorNo', { defaultValue: 'Διαχωρισμός: ❌' });
  const demoFooterText = activeActivity === '4'
    ? `${t('neural.separator', { defaultValue: 'Διαχωρισμός' })}: ${thresholdDisplayText} | ${separableLabel}`
    : '';
  ...
  return (
    <TeacherCard title={heroTitle}>
      <div className="connection-status">
        ...
        <strong>{isSocketConnected ? t('common.connected') : t('common.disconnected')}</strong>
        ...
        editingName ? (
          ...
          >
            {t('neural.studentNameLabel', { defaultValue: 'όνομα' })}: {studentName}
          </span>
        )
      )}
        {!isStudent && <span>{t('neural.connectedCount', { defaultValue: 'συνδεδεμένοι' })}: {roster.length}</span>}
      </div>

      {isScreen && (
        <>
          <div className="screen-top-bar">
            <strong>{t('neural.classView', { defaultValue: 'Προβολή τάξης' })}</strong>
          ...
          <div className="operation-tree" aria-label={t('neural.operationTree', { defaultValue: 'Δέντρο πράξεων' })}>
