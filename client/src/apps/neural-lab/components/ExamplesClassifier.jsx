      ...
      <div className="data-section">
              ...
              <tr key={idx}>
                ...
                {showResultDetails && (
                  <td className={threshold ? (ex.result ? 'result-positive' : 'result-negative') : ''}>
                    {threshold ? (ex.result ? `✅ ${t('neural.positive')}` : `❌ ${t('neural.negative')}`) : ''}
                  </td>
                )}
              </tr>
            ))}
        ...
        <table className="data-table">
            ...
            <tr>
                ...
                {t('neural.confusionMatrix')}
                </th>
            </tr>
            <tr>
                {/* 2η γραμμή: Οι κύριες κατηγορίες στηλών */}
                <th rowSpan="2">{t('neural.actualClass')}</th>
                <th colSpan="2">{t('neural.prediction')}</th>
            </tr>
            <tr>
                {/* 3η γραμμή: Οι υποκατηγορίες της πρόβλεψης */}
                <th>{t('neural.positive')}</th>
                <th>{t('neural.negative')}</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>{t('neural.positive')}</td>
                <td className="result-positive">TP: {metricValue('tp')}</td>
                <td className="result-negative">FN: {metricValue('fn')}</td>
            </tr>
            <tr>
                <td>{t('neural.negative')}</td>
        ...
        <div className="student-inline-note" style={{ marginTop: '0.6rem' }}>
          {t('neural.currentWeights')}: w1 = {safeWeights.w1}, w2 = {safeWeights.w2}
          {showResultDetails && threshold && ` | ${t('neural.separator')}: ${threshold.op} ${threshold.boundary}`}
        </div>
      </div>
    </Accordion>
  );
