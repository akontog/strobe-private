(function initializePresentationShell() {
  ...
  function initDeck(config = {}) {
    ...
    function setSidebarOpen(forceOpen = null) {
      ...
      if (contentEl) {
        contentEl.classList.toggle("expanded", !shouldOpen);
      }
    ...
    const initialSidebarOpen = typeof config.initialSidebarOpen === "boolean"
      ? config.initialSidebarOpen
      : !Boolean(config.menuButtonTogglesSidebar);

    if (sidebarEl) {
      setSidebarOpen(initialSidebarOpen);
    }

    // Keep home button hidden until user opens the sidebar via menu button.
    if (homeButtonEl) {
      homeButtonEl.classList.toggle("visible", Boolean(initialSidebarOpen));
    }
