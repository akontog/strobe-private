...
- Στη διαφάνεια 10.3 οι students μπορούν να στείλουν random packs συχνοτήτων.
...
- 7. Μετασχηματισμός Fourier: 7.1-7.8
- 8. Πολυώνυμα και συνέλιξη: 8.1-8.3
- 9. Cooley-Tukey και πολυπλοκότητα: 9.1-9.3
- 10. Pipeline θάλασσας: 10.1-10.9
- 11. Εκτέλεση και κλείσιμο: 11.1-11.2
...
- 4.1 e^(x/2)cos(x) με πολυώνυμο 2ου βαθμού
- 4.2 Taylor του cos(x)
- 4.3 Taylor του sin(x)
- 4.4 Taylor του e^x
- 6.1 Fourier και θερμοκρασία ράβδου
- 7.1 Δραστηριότητα: Μίξη συχνοτήτων
- 7.2 Από το σήμα στο φάσμα
- 7.3 FFT Duel
- 7.4 Σύνθετο σήμα και spikes
- 7.7 Παράδειγμα Fourier -> IFFT σε ήχο
- 7.8 Δραστηριότητα: Επιλογή φίλτρου
- 9.1 Cooley-Tukey: divide and conquer
- 9.2 Cooley-Tukey FFT: αναδρομή και επίπεδα
- 9.3 Αγώνες πολυπλοκότητας
...
#### 4.1 Προσέγγιση του e^(x/2)cos(x) με πολυώνυμο 2ου βαθμού
Διαδραστική προσαρμογή του πολυωνύμου \(c_0 + c_1x + c_2x^2\) με sliders (ξεκινώντας από 0) για τους συντελεστές και σύγκριση με την \(e^{x/2}\cos(x)\). Κουμπιά "Βέλτιστη λύση" θέτουν τις τιμές στις προσεγγίσεις Taylor (c₀=1, c₁=1/2, c₂=-3/8). Εμφανίζονται οι παράγωγοι έως τη 3η τάξη και ο γενικός τύπος Taylor.
...
#### 7.7 Παράδειγμα Fourier -> IFT σε ήχο
...
Οπτικοποίηση της τιμής πολυωνύμου από τους συντελεστές του.

#### 8.2 Πολυώνυμο από σημεία
Η ίδια συνάρτηση μέσα από αναπαράσταση σημείων.

#### 8.3 Πολλαπλασιασμός πολυωνύμων
Διαδραστική εικόνα της συνέλιξης συντελεστών.

### 9. Cooley-Tukey και πολυπλοκότητα
...
#### 10.7 IFFT: από `H(kx,kz)` σε `h(x,z,t)`
...
#### 11.1 Εκτέλεση κάθε animation ξεχωριστά
Πρακτικά βήματα χρήσης και δοκιμής των demos.

#### 11.2 Θάλασσα κυμάτων (τελική εικόνα)
